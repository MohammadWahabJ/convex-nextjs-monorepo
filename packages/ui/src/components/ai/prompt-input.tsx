...
import { Loader } from "@workspace/ui/components/ai/loader";
...
export type PromptInputAttachmentProps = HTMLAttributes<HTMLDivElement> & {
  data: FileUIPart & { id: string; uploading?: boolean };
  className?: string;
};
...
export const PromptInput = ({
  ...
  const matchesAccept = useCallback(
    (f: File) => {
      ...
      // Split accept string by comma and trim each part
    ...
    : (id: string) =>
        setItems((prev) => {
          ...
          if (found?.url) {
            ...
            blobUrlsRef.current.delete(found.url);
          }
...
> & {
  ...
  onTranscribeAudio?: (audio: Uint8Array, languageCode?: string) => Promise<string>;
};
...
export const PromptInputSpeechButton = ({
      ...
      recorder.onstop = async () => {
        try {
          ...
          if (!onTranscribeAudio) {
            console.error("No transcription function provided");
            return;
          }
          
          const transcribedText = await onTranscribeAudio(
            new Uint8Array(arrayBuffer),
            "de" // Default to German
          );

          if (transcribedText && textareaRef?.current) {
            const textarea = textareaRef.current;
            const currentValue = textarea.value;
            const newValue = currentValue + (currentValue ? " " : "") + transcribedText;
  ...
  const toggleRecording = useCallback(async () => {
    if (isRecording) {
      ...
      if (!mediaRecorder) {
        ...
        // Wait a bit for the MediaRecorder to be set
        setTimeout(() => {
  ...
  const getButtonClassName = () => {
    if (isRecording) {
      return "animate-pulse bg-red-500 text-white hover:bg-red-600";
    }
    if (isTranscribing) {
